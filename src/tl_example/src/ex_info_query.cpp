 ...
 *
 * @see ex_driver_quick_test.cpp — 遍历测试 tl_driver 全部接口的驱动快速自检
 */
...
{
public:
  explicit InfoQueryDemo(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
    : Node("ex_info_query", options)
  {
    ...
    get_joint_param_cli_        = create_client<tl_ros2_interface::srv::GetRobotJointParam>("/tl_driver/get_robot_joint_param");
    get_joint_temp_cli_         = create_client<tl_ros2_interface::srv::GetJointTemperature>("/tl_driver/get_joint_temperature");
    ...
    get_joint_sw_cli_           = create_client<tl_ros2_interface::srv::GetJointSoftwareVersion>("/tl_driver/get_joint_software_version");
    ...
    get_motor_torque_cli_       = create_client<tl_ros2_interface::srv::GetCurrentMotorTorque>("/tl_driver/get_current_motor_torque");
    get_line_speed_cli_         = create_client<tl_ros2_interface::srv::GetCurrentLineJointSpeed>("/tl_driver/get_current_line_joint_speed");
...
private:
  ...
  bool callTrigger(const std::string& key, const std::string& label)
  {
    ...
    if (it == trig_clis_.end()) return false;
    auto cli = it->second;
...
void tl_example::InfoQueryDemo::testInfoQuery()
{
  ...
  spinWaitFor([this]() { return got_joint_state_; }, 2.0);
  ...
  spinWaitFor([this]() { return got_tcp_pose_; }, 2.0);
  if (last_tcp_pose_) {
    RCLCPP_INFO(this->get_logger(), "      pos = [%.3f, %.3f, %.3f] mm",
                last_tcp_pose_->position.x, last_tcp_pose_->position.y, last_tcp_pose_->position.z);
    RCLCPP_INFO(this->get_logger(), "      rpy = [%.3f, %.3f, %.3f] rad",
                last_tcp_pose_->rpy.x, last_tcp_pose_->rpy.y, last_tcp_pose_->rpy.z);
  } else {
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.3] 查询运行速度 get_speed ---");
  {
    ...
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      speed = %.1f%%", resp->speed);
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.5] 查询机械臂状态 get_robot_state ---");
  {
    ...
    req->channel = 1; req->stop = false; req->mode = 0; req->interval = 100;
    req->io_state = false; req->position = 0; req->detail_motion_pos = false; req->pos_sum = 1;
    auto resp = callService<tl_ros2_interface::srv::GetRobotState>(get_robot_state_cli_, req, "get_robot_state");
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      信息: %s", resp->message.c_str());
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.7] 查询关节参数 get_robot_joint_param ---");
  {
    ...
    req->id = 1;
    auto resp = callService<tl_ros2_interface::srv::GetRobotJointParam>(get_joint_param_cli_, req, "get_robot_joint_param(id=1)");
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.8] 查询关节温度 get_joint_temperature ---");
  {
    ...
    auto resp = callService<tl_ros2_interface::srv::GetJointTemperature>(get_joint_temp_cli_, req, "get_joint_temperature");
    if (resp && resp->success) printArray(resp->temperatures, "      temperature(℃) ");
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.10] 查询电机电流 get_motor_current ---");
  {
    ...
    if (resp && resp->success) printArray(resp->motor_current, "      motor_current(A) ");
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.11] 查询关节软件版本 get_joint_software_version ---");
  {
    ...
    req->axis_num = 1;
    auto resp = callService<tl_ros2_interface::srv::GetJointSoftwareVersion>(get_joint_sw_cli_, req, "get_joint_software_version(axis=1)");
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      版本: %s", resp->message.c_str());
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.13] 查询当前坐标系 get_current_coord ---");
  {
    ...
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      coord = %d（0=关节 1=直角 2=工具 3=用户）", resp->coord);
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.14] 查询坐标系编号 get_coord_num ---");
  {
    ...
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      tool_num=%d user_num=%d", resp->tool_num, resp->user_num);
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.15] 查询机械臂DH参数 get_dh_param ---");
  {
    ...
    if (resp && resp->success) {
      RCLCPP_INFO(this->get_logger(), "      l1=%.3f l2=%.3f l3=%.3f l4=%.3f", resp->param.l1, resp->param.l2, resp->param.l3, resp->param.l4);
    }
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.16] 查询所有作业文件 get_all_job_filename ---");
  {
    ...
    if (resp && resp->success) {
      ...
      for (const auto& group : resp->robots_file) {
        for (const auto& name : group.file_name) RCLCPP_INFO(this->get_logger(), "      作业[%d]: %s", ++idx, name.c_str());
      }
    }
  }
  ...
  //           [4]=用户序号 [5][6]=备用 [7~13]=点位信息
  {
    ...
    req->pos = pos;
    req->move_type = "MOVJ";
    auto resp = callService<tl_ros2_interface::srv::GetPosReachable>(get_pos_reachable_cli_, req, "get_pos_reachable(MOVJ)");
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      ✓ 目标可达");
  }
  {
    ...
    req->pos = pos;
    req->move_type = "MOVL";
    auto resp = callService<tl_ros2_interface::srv::GetPosReachable>(get_pos_reachable_cli_, req, "get_pos_reachable(MOVL)");
    if (resp && resp->success) RCLCPP_INFO(this->get_logger(), "      ✓ 目标可达");
  }

  // [4.18] 查询机械臂运行状态（话题 /arm_status）
  RCLCPP_INFO(this->get_logger(), "  --- [4.18] 查询机械臂运行状态 /arm_status ---");
  spinWaitFor([this]() { return arm_run_state_ != "unknown"; }, 2.0);
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.19] 查询当前电机力矩 get_current_motor_torque ---");
  {
    ...
    auto resp = callService<tl_ros2_interface::srv::GetCurrentMotorTorque>(get_motor_torque_cli_, req, "get_current_motor_torque");
    if (resp && resp->success) printArray(resp->motor_torque, "      motor_torque(Nm) ");
  }
  ...
  RCLCPP_INFO(this->get_logger(), "  --- [4.20] 查询当前线速度和关节速度 get_current_line_joint_speed ---");
  {
    ...
    auto resp = callService<tl_ros2_interface::srv::GetCurrentLineJointSpeed>(get_line_speed_cli_, req, "get_current_line_joint_speed");
    if (resp && resp->success) {
