 ...
 */
std::vector<double> makePos(double x, double y, double z,
                            double rx = TOOL_DOWN_RX, double ry = TOOL_RY,
                            double rz = TOOL_RZ) {
...
MedicalDemo::MedicalDemo(const rclcpp::NodeOptions &options)
    : Node("medical_demo", options) {
  movel_pub_ = this->create_publisher<MoveCommand>("/tl_driver/moveL", 10);

  state_timer_ =
      this->create_wall_timer(std::chrono::milliseconds(100),
                              std::bind(&MedicalDemo::stateMachine, this));

  arm_status_sub_ = this->create_subscription<ArmStatus>(
      "/arm_status", 10,
      std::bind(&MedicalDemo::onArmStatus, this, std::placeholders::_1));

  RCLCPP_INFO(this->get_logger(),
              "============================================================");
  RCLCPP_INFO(this->get_logger(),
              "  队列 MoveL 演示节点已启动 (Queue MoveL Demo)");
...
void MedicalDemo::ensureClients() {
  ...
  if (!set_mode_cli_) {
    set_mode_cli_ =
        this->create_client<SetCurrentMode>("/tl_driver/set_current_mode");
  }
  if (!power_on_cli_) {
...
bool MedicalDemo::waitService(rclcpp::ClientBase::SharedPtr client,
                              const std::string &name, double timeout_s) {
  if (!client->wait_for_service(std::chrono::duration<double>(timeout_s))) {
    RCLCPP_ERROR(this->get_logger(), "服务 %s 未就绪（%.0fs 超时）",
...
MedicalDemo::MoveCommand MedicalDemo::buildMoveL(const std::string &pos_key,
...
void MedicalDemo::startConnect() {
  ...
  auto future = connect_cli_->async_send_request(
      req, std::bind(&MedicalDemo::onConnectDone, this, std::placeholders::_1));
...
void MedicalDemo::startSetMode() {
  ...
  auto future = set_mode_cli_->async_send_request(
      req, std::bind(&MedicalDemo::onSetModeDone, this, std::placeholders::_1));
  state_ = State::DONE;
}

void MedicalDemo::onSetModeDone(
    rclcpp::Client<SetCurrentMode>::SharedFuture future) {
  const auto &resp = future.get();
  if (resp->success) {
    RCLCPP_INFO(this->get_logger(), "✓ 已切换到示教模式 (mode=0)");
    state_ = State::POWER_ON;
  } else {
    RCLCPP_ERROR(this->get_logger(), "✗ 切换示教模式失败: %s",
                 resp->message.c_str());
    state_ = State::DONE;
  }
}
...
void MedicalDemo::startPowerOn() {
  auto req = std::make_shared<Trigger::Request>();
  auto future = power_on_cli_->async_send_request(
      req, std::bind(&MedicalDemo::onPowerOnDone, this, std::placeholders::_1));
  state_ = State::DONE;
}
...
void MedicalDemo::startSetSpeed() {
  ...
  auto future = set_speed_cli_->async_send_request(
      req,
      std::bind(&MedicalDemo::onSetSpeedDone, this, std::placeholders::_1));
  state_ = State::DONE;
}

void MedicalDemo::onSetSpeedDone(
    rclcpp::Client<SetSpeed>::SharedFuture future) {
  const auto &resp = future.get();
  if (resp->success) {
    ...
    RCLCPP_ERROR(this->get_logger(), "✗ 设置速度失败: %s",
                 resp->message.c_str());
    state_ = State::DONE;
  }
}
...
void MedicalDemo::moveNextStep() {
  ...
  case MovePhase::IDLE: {
    ...
    RCLCPP_INFO(this->get_logger(), "▶ [%zu/%zu] %s  → 位置=%s  速度=%.0f%%",
                seq_index_ + 1, DEMO_SEQUENCE.size(), step.description.c_str(),
                step.pos_key.c_str(), step.velocity);
  ...
  if (move_phase_ != MovePhase::IDLE) {
    ...
    if (elapsed > safety_timeout_s_) {
      RCLCPP_WARN(
          this->get_logger(), "步骤 %zu/%zu 安全超时（%.1fs > %.0fs），跳过",
          seq_index_ + 1, DEMO_SEQUENCE.size(), elapsed, safety_timeout_s_);
      seq_index_++;
      move_phase_ = MovePhase::IDLE;
    }
  }
...
void MedicalDemo::startPowerOff() {
  ...
  auto req = std::make_shared<Trigger::Request>();
  auto future = power_off_cli_->async_send_request(
      req,
      std::bind(&MedicalDemo::onPowerOffDone, this, std::placeholders::_1));
  state_ = State::DONE;
}
