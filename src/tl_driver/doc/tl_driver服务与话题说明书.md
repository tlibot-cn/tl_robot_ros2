...
* 3.19[查询当前电机力矩](#查询当前电机力矩)
* 3.20[查询当前线速度和关节速度](#查询当前线速度和关节速度)
...
| 返回值 | true-查询成功，false-查询失败<br>查询成功时返回当前电机力矩<br>int32[] motor_torque：机械臂本体当前电机力矩<br>int32[] motor_torque_sync：同步轴当前电机力矩 |
#### 命令示例
```
ros2 service call /tl_driver/get_current_motor_torque tl_ros2_interface/srv/GetCurrentMotorTorque "{}"
```
### 查询当前线速度和关节速度
| 功能描述 | 查询机械臂当前线速度和关节速度 |
| :---: | :---- |
| 通信机制 | ROS2服务 |
| 参数说明 | GetCurrentLineJointSpeed.srv |
| 返回值 | true-查询成功，false-查询失败<br>查询成功时返回当前线速度和关节速度<br>float64 line_speed：当前线速度<br>float64[] joint_speed：机械臂本体当前关节速度<br>float64[] joint_speed_sync：同步轴当前关节速度 |
#### 命令示例
```
