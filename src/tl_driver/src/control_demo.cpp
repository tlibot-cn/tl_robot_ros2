...
public:
  ...
  : Node("servoj_control_demo")
  {
    ...
    servoj_pos_pub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
      "/tl_driver/set_servoj_pos", 10);
    
    // 创建订阅者 - 订阅目标笛卡尔位姿
    target_pose_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      "/target_pos_cartesian",
      10,
      std::bind(&ControlNode::target_pose_callback, this, std::placeholders::_1));
...
private:
  void open_servoj()
  {
    ...
    auto result = open_servoj_client_->async_send_request(
      request,
      std::bind(&ControlNode::open_servoj_callback, this, std::placeholders::_1));
  }
  
  void open_servoj_callback(
    rclcpp::Client<tl_ros2_interface::srv::OpenServoJ>::SharedFuture future)
  {
    auto response = future.get();
    
    if (response->success)
    {
      RCLCPP_INFO(this->get_logger(), "ServoJ opened successfully: %s", 
                  response->message.c_str());
    }
    else
    {
      RCLCPP_ERROR(this->get_logger(), "Failed to open servoJ: %s", 
                   response->message.c_str());
    }
  }
  ...
  void close_servoj()
  {
    ...
    if (future == std::future_status::ready)
    {
      ...
      if (response->success)
      {
        RCLCPP_INFO(this->get_logger(), "ServoJ closed successfully: %s", 
                    response->message.c_str());
      }
      else
      {
        RCLCPP_ERROR(this->get_logger(), "Failed to close servoJ: %s", 
                     response->message.c_str());
      }
    }
    else
  ...
  void target_pose_callback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
  {
    ...
    if (msg->data.size() < 6)
    {
      RCLCPP_WARN(this->get_logger(), 
        "Invalid input data size: %zu, expected at least 6 values [x, y, z, rx, ry, rz]",
        msg->data.size());
      return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), 
      "Received target pose: x=%.3f, y=%.3f, z=%.3f, rx=%.2f, ry=%.2f, rz=%.2f",
    ...
    auto result = coord_transform_client_->async_send_request(
      request,
      std::bind(&ControlNode::transform_callback, this, 
                std::placeholders::_1));
  }
    ...
    rclcpp::Client<tl_ros2_interface::srv::CoordTransform>::SharedFuture future)
  {
    ...
    if (!response->success)
    {
      RCLCPP_ERROR(this->get_logger(), "Coordinate transform failed: %s", 
                   response->message.c_str());
      return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Transform success, got %zu joint angles (degrees)", 
                joint_angles_deg.size());
    ...
    for (size_t i = 0; i < joint_angles_deg.size(); ++i)
    {
      ...
      if (i < joint_angles_deg.size() - 1) ss << ", ";
    }
    ss << "] deg";
