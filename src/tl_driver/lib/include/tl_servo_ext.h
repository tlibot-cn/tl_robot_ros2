 ...
 *
 * @param socket_udp UDP socket（7000 伺服端口）
 ...
 */
TL_API Result open_servo_movej(SOCKETFD socket_udp, double vmax_pct = 30.0, double amax_pct = 30.0, double jmax_pct = 30.0);
 ...
 */
TL_API Result close_servo_movej(SOCKETFD socket_udp);
 ...
 */
TL_API Result open_servo_movel(SOCKETFD socket_udp, double vmax_pct = 30.0, double amax_pct = 30.0, double jmax_pct = 30.0);
 ...
 * @note 引用计数归零时停止后台发送线程并关闭 servoJ
 */
TL_API Result close_servo_movel(SOCKETFD socket_udp);
 ...
 *
 * @param socket_tcp TCP socket（6001 控制端口）：仅用于查询当前关节角/逆解
 * @param socket_udp UDP socket（7000 伺服端口）：servoJ 数据发送
 * @param target 目标关节角，6 或 7 个元素（7 轴含真实 J7；6 轴时第 7 位补 0）
 * @param coord 0=关节角直接插值（默认）/ 1=基坐标系（先逆解转关节角）
 ...
 * @note 推荐使用枚举重载（Coord::JOINT / Coord::BASE），避免魔法数字
 */
TL_API Result servo_movej(SOCKETFD socket_tcp, SOCKETFD socket_udp, const std::vector<double>& target, int coord = 0,
                   double step_size = 0.72);

/**
 * @brief Servo MoveJ 关节空间伺服运动（枚举坐标系重载）
 *
 * 语义与 int 版本一致，仅 coord 参数以 Coord 枚举传入。
 *
 * @param socket_tcp TCP socket（6001 控制端口）：仅用于查询当前关节角/逆解
 * @param socket_udp UDP socket（7000 伺服端口）：servoJ 数据发送
 * @param target 目标关节角，6 或 7 个元素（7 轴含真实 J7；6 轴时第 7 位补 0）
 * @param coord 坐标系，仅支持 Coord::JOINT（关节角直接插值）/ Coord::BASE（基坐标系，先逆解转关节角）
 * @param step_size 插值步长（关节角，度），<=0 时使用默认 0.72
 * @return SUCCESS / OPERATION_NOT_ALLOWED（未先 open_servo_movej）/
 *         DISCONNECT（未连接）/ PARAM_ERR（参数错误）
 *
 * @warning 需先调用 open_servo_movej 开启跟踪模式
 */
TL_API Result servo_movej(SOCKETFD socket_tcp, SOCKETFD socket_udp, const std::vector<double>& target, Coord coord,
                   double step_size = 0.72);
 ...
 *
 * @param socket_tcp TCP socket（6001 控制端口）：仅用于查询当前位姿/逆解
 * @param socket_udp UDP socket（7000 伺服端口）：servoJ 数据发送
 * @param target_pose 目标位姿 [X,Y,Z,RX,RY,RZ]（mm, rad），6 或 7 个元素
 * @param coord 1=基坐标系直线插值（默认）/ 0=关节角模式（按关节角直线插值）
 * @param step_size 插值步长（关节角，度），<=0 时使用默认 0.72
 ...
 * @note 推荐使用枚举重载（Coord::JOINT / Coord::BASE），避免魔法数字
 */
TL_API Result servo_movel(SOCKETFD socket_tcp, SOCKETFD socket_udp, const std::vector<double>& target_pose, int coord = 1,
                   double step_size = 2.0);

/**
 * @brief Servo MoveL 笛卡尔直线伺服运动（枚举坐标系重载）
 *
 * 语义与 int 版本一致，仅 coord 参数以 Coord 枚举传入。
 *
 * @param socket_tcp TCP socket（6001 控制端口）：仅用于查询当前位姿/逆解
 * @param socket_udp UDP socket（7000 伺服端口）：servoJ 数据发送
 * @param target_pose 目标位姿 [X,Y,Z,RX,RY,RZ]（mm, rad），6 或 7 个元素
 * @param coord 坐标系，仅支持 Coord::JOINT（按关节角直线插值）/ Coord::BASE（基坐标系直线插值）
 * @param step_size 插值步长（关节角，度），<=0 时使用默认 0.72
 * @return SUCCESS / OPERATION_NOT_ALLOWED（未先 open_servo_movel）/
 *         DISCONNECT（未连接）/ PARAM_ERR（参数错误）/ 逆解失败错误码
 *
 * @warning 需先调用 open_servo_movel 开启跟踪模式
 */
TL_API Result servo_movel(SOCKETFD socket_tcp, SOCKETFD socket_udp, const std::vector<double>& target_pose, Coord coord,
                   double step_size = 2.0);
 ...
 * @param socketFd UDP socket（7000 伺服端口）
