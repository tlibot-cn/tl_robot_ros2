 ...
 */
TL_API Result open_servo_movej(SOCKETFD socket_udp, double vmax_pct = 30.0, double amax_pct = 30.0,
 ...
 */
TL_API Result open_servo_movel(SOCKETFD socket_udp, double vmax_pct = 30.0, double amax_pct = 30.0,
                               double jmax_pct = 30.0);
 ...
 */
TL_API Result servo_movej(SOCKETFD socket_tcp, SOCKETFD socket_udp,
                          const std::vector<double>& target, int coord = 0,
 ...
 * @param socket_udp UDP socket（7000 伺服端口）：servoJ 数据发送
 * @param target 目标关节角，6 或 7 个元素（7 轴含真实 J7；6 轴时第 7 位补 0）
 * @param coord 坐标系，仅支持 Coord::JOINT（关节角直接插值）/
 * Coord::BASE（基坐标系，先逆解转关节角）
 * @param step_size 插值步长（关节角，度），<=0 时使用默认 0.72
 * @return SUCCESS / OPERATION_NOT_ALLOWED（未先 open_servo_movej）/
 *         DISCONNECT（未连接）/ PARAM_ERR（参数错误）
 *
 ...
 */
TL_API Result servo_movej(SOCKETFD socket_tcp, SOCKETFD socket_udp,
                          const std::vector<double>& target, Coord coord, double step_size = 0.72);
 ...
 * @note 推荐使用枚举重载（Coord::JOINT / Coord::BASE），避免魔法数字
 */
TL_API Result servo_movel(SOCKETFD socket_tcp, SOCKETFD socket_udp,
                          const std::vector<double>& target_pose, int coord = 1,
 ...
 */
TL_API Result servo_movel(SOCKETFD socket_tcp, SOCKETFD socket_udp,
                          const std::vector<double>& target_pose, Coord coord,
                          double step_size = 2.0);
