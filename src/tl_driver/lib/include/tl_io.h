 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR 参数错误（如端口号越界）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR 参数错误（如数值越界）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * @param enable
 * 输出：是否复位容器，大小为所有IO板输出端口数，从第二块IO板开始，每一块IO板的起始位置为上一块IO板的末位端口的顺延
 * @param value
 * 输出：复位值容器，大小为所有IO板输出端口数，从第二块IO板开始，每一块IO板的起始位置为上一块IO板的末位端口的顺延
 * @deprecated 请使用 IoResetType 枚举重载版本
 * @note 推荐使用枚举重载（IoResetType::REMOTE_IO / IoResetType::MODE_STOP /
 * IoResetType::PROGRAM_ERROR），避免魔法数字
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR 参数错误；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * @param robotNum 机器人编号(1-4)
 * @param type IoResetType::REMOTE_IO（远程IO复位）/
 * IoResetType::MODE_STOP（切模式停止）/ IoResetType::PROGRAM_ERROR（程序报错）
 * @param enable
 * 输出：是否复位容器，大小为所有IO板输出端口数，从第二块IO板开始，每一块IO板的起始位置为上一块IO板的末位端口的顺延
 * @param value
 * 输出：复位值容器，大小为所有IO板输出端口数，从第二块IO板开始，每一块IO板的起始位置为上一块IO板的末位端口的顺延
 ...
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result get_IO_reset_function(SOCKETFD socketFd, int robotNum, IoResetType type,
                                    std::vector<int>& enable, std::vector<int>& value);
