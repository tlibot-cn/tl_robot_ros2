...
/**
 * @brief 设置灵巧手参数
 * @param param 灵巧手参数（mode 通讯模式 0:modbus_rtu 1:RS485 / speed 速度 /
 * force_threshold 按指力阈值，结构体 DexterousHandsParam 详见 tl_types.h）
 ...
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result set_dexterous_hands_force_threshold(SOCKETFD socketFd, int mode, int speed,
                                                  const ForceThreshold& force_threshold);
 ...
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result set_gesture_serial_number(SOCKETFD socketFd, int gesture_serial_number, int mode,
                                        int speed, int process_number);
 ...
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result set_action_serial_number(SOCKETFD socketFd, int action_serial_number, int mode,
                                       int speed, int process_number);
