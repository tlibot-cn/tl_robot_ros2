 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR 参数错误（如作业文件名不合法）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR 参数错误（如文件不存在）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR 参数错误（如目录不存在）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR
 * 参数错误（如行号越界或参数非法）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * @param line 插入的行号
 * @param moveCmd 运动指令参数，详见 MoveCmd；增量指令需
 * targetPosType=3（变量）、targetPosName="RP0001"（控制器协议约定）
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT
 * 未连接；-3=PARAM_ERR
 * 参数错误（如行号越界或参数非法）；-4=OPERATION_NOT_ALLOWED
 * 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
