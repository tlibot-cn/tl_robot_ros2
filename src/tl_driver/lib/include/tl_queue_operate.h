 ...
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT 未连接；-3=PARAM_ERR
 * 参数错误（传入的size超过队列长度）；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION
 * 异常；-6=TIMEOUT 超时
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API TL_DEPRECATED("使用 queue_motion_stop_not_power_off 替代") Result
    queue_motion_stop(SOCKETFD socketFd);
...
/**
 * @brief 队列运动模式的本地队列最后插入一条moveS曲线运动
 * @param moveCmd 运动指令，详见 MoveCmd
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT 未连接；-3=PARAM_ERR
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_moveCA(SOCKETFD socketFd, MoveCmd moveCmd);
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_imove(SOCKETFD socketFd, MoveCmd moveCmd);

/**
 * @brief 队列运动模式的本地队列最后插入一条SAMOV(定点移动)指令
 * @param moveCmd 运动指令结构体
 * @param posData 点位数据
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT 未连接；-3=PARAM_ERR
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_samov(SOCKETFD socketFd, MoveCmd moveCmd,
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_cil(SOCKETFD socketFd, MoveCmd moveCmd, int id);
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_moveComm(SOCKETFD socketFd, std::string moveType, double m_vel,
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_until(
    SOCKETFD socketFd, const std::vector<std::vector<Condition>>& conditionGroups,
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_end_until(SOCKETFD socketFd);
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_while(
    SOCKETFD socketFd, const std::vector<std::vector<Condition>>& conditionGroups,
    const std::vector<int>& logic, const std::vector<std::vector<int>>& logicGroup);

/**
 * @brief 队列运动模式的本地队列最后插入一条ENDWHILE(结束循环)指令
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT 未连接；-3=PARAM_ERR
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_end_while(SOCKETFD socketFd);
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_if(SOCKETFD socketFd,
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_end_if(SOCKETFD socketFd);
 ...
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_label(SOCKETFD socketFd, const std::string& label);
 ...
 * @param logic 第一层的逻辑类型，0为"与"，1为"或"
 * @param logicGroup 第二层的逻辑类型，0为"与"，1为"或"
 * @param jumpConditionFlag 是否使用跳转条件
 * @param label 跳转目标标签名字
 * @return 0=SUCCESS 成功；-1=RECEIVE_FAILED 接收失败；-2=DISCONNECT 未连接；-3=PARAM_ERR
 * 参数错误；-4=OPERATION_NOT_ALLOWED 操作不允许；-5=EXCEPTION 异常；-6=TIMEOUT 超时
 */
TL_API Result queue_motion_push_back_jump(
    SOCKETFD socketFd, const std::vector<std::vector<Condition>>& conditionGroups,
    const std::vector<int>& logic, const std::vector<std::vector<int>>& logicGroup,
