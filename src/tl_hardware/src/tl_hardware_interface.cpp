...
TLHardwareInterface::TLHardwareInterface()
    : logger_(rclcpp::get_logger("tl_hardware_interface")),
      ...
      last_read_time_(0, 0, RCL_ROS_TIME) {}

TLHardwareInterface::~TLHardwareInterface() { stop_executor(); }

hardware_interface::CallbackReturn
TLHardwareInterface::on_init(const hardware_interface::HardwareInfo &info) {
  ...
  joint_states_topic_ =
      get_hardware_parameter("joint_states_topic", "/joint_states");

  servoj_topic_ =
      get_hardware_parameter("servoj_topic", "/tl_driver/set_servoj_pos");

  open_servoj_service_ =
      get_hardware_parameter("open_servoj_service", "/tl_driver/open_servoj");

  close_servoj_service_ =
      get_hardware_parameter("close_servoj_service", "/tl_driver/close_servoj");
  ...
  auto parse_vector_param = [&](const std::string &name,
                                double default_val) -> std::vector<double> {
    ...
    if (raw.find(',') != std::string::npos) {
      ...
      for (size_t pos = 0, end = 0; end != std::string::npos; pos = end + 1) {
        ...
        try {
          ...
          RCLCPP_WARN(
              logger_,
              "Invalid value '%s' in parameter '%s', using default %.1f",
      ...
      if (result.size() != joint_count) {
        RCLCPP_WARN(
            logger_,
            "Parameter '%s' has %zu values but %zu joints, using default %.1f",
      ...
      try {
        ...
        RCLCPP_WARN(logger_,
                    "Invalid value '%s' for parameter '%s', using default %.1f",
                    raw.c_str(), name.c_str(), default_val);
      }
    }
    return result;
  };
  ...
  try {
    ...
    RCLCPP_WARN(logger_, "Invalid state_timeout_sec '%s', using default 1.0",
                state_timeout.c_str());
  ...
  RCLCPP_INFO(logger_, "Close servoj service: %s",
              close_servoj_service_.c_str());
...
hardware_interface::CallbackReturn TLHardwareInterface::on_configure(
  ...
  try {
    ...
    // Publisher for servoj position commands (Float64MultiArray in degrees).
    servoj_pos_pub_ = node_->create_publisher<std_msgs::msg::Float64MultiArray>(
        servoj_topic_, rclcpp::QoS(10).reliable());

    // Service clients for servoj lifecycle.
    open_servoj_client_ =
        node_->create_client<tl_ros2_interface::srv::OpenServoJ>(
            open_servoj_service_);

    close_servoj_client_ =
        node_->create_client<std_srvs::srv::Trigger>(close_servoj_service_);
    ...
    RCLCPP_ERROR(logger_, "Failed to configure hardware interface: %s",
                 e.what());
    stop_executor();
    return hardware_interface::CallbackReturn::ERROR;
  }
...
hardware_interface::CallbackReturn TLHardwareInterface::on_deactivate(
  ...
  if (close_servoj_client_ && close_servoj_client_->wait_for_service(5s)) {
    ...
    if (status != std::future_status::ready) {
      ...
      auto response = future.get();
      if (!response->success) {
        RCLCPP_WARN(logger_, "close_servoj failed: %s",
...
TLHardwareInterface::read(const rclcpp::Time & /*time*/,
  ...
  if (time_since_update.seconds() > state_timeout_sec_) {
    ...
    if (now_steady - last_warning >= 5s) {
      RCLCPP_WARN(logger_, "No joint state update for %.3f seconds",
...
hardware_interface::return_type
TLHardwareInterface::write(const rclcpp::Time &time,
                           const rclcpp::Duration & /*period*/) {
...
PLUGINLIB_EXPORT_CLASS(tl_hardware::TLHardwareInterface,
