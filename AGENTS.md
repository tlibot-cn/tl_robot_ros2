...
仅当**全部为否**（纯内部重构：格式化、注释、命名统一）才可跳过文档同步；但属"清理/迁移"类变更仍需在 `CHANGELOG.md` 记录一行。
...
- 发版时把 `[Unreleased]` 内容合并进版本号章节并按类型归类，然后重置 `[Unreleased]`；文件头部保留一行「最新发布版本：`[x.y.z]`（日期，git tag `Vx.y.z`）」标明当前发版点
...
- 文档改动必须与代码改动**成对出现在同一提交**：只有代码没有文档 = 变更未完成，禁止提交。
