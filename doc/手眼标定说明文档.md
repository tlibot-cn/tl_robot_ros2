...
`calib_node` 输出的是 `R_cam2tool`（相机→末端 的旋转矩阵），control_node 中使用的是 `T_tool_camera`（末端→相机 的齐次变换矩阵）：

```python
import numpy as np
from scipy.spatial.transform import Rotation as R

# 从 calib_node 结果中获取 rotation_matrix 和 translation_vector
R_cam2tool = np.array(rotation_matrix)
t_cam2tool = np.array(translation_vector).reshape(3, 1)

# 求逆得到 T_tool_camera
R_tool2cam = R_cam2tool.T
t_tool2cam = -R_tool2cam @ t_cam2tool

# 拼接为 4×4 齐次变换矩阵
T_tool_camera = np.eye(4)
T_tool_camera[:3, :3] = R_tool2cam
T_tool_camera[:3, 3] = t_tool2cam.flatten()

# 按行优先展开为 16 元素列表，填入 control_node.yaml
handeye_matrix = T_tool_camera.flatten().tolist()
print(handeye_matrix)
```
