...
| `build/` `install/` | 241M / 182M | colcon 构建与安装产物 |
| `.gitnexus/` | 40M | 代码图谱索引 |
| `.codegraph/` | 7.0M | 图库 |
| `log/` | 12M | 构建日志 |
...
| `src/tl_driver/lib/` | 4.4M | `arm/libtl_host.so`、`x86/libtl_host.so`、SDK 头 `include/*.h`（14 个头文件，约 3.5k 行，纯文本） |
| `src/tl_teleop/lib/include/` | — | PXREA SDK 头（纯文本，值得直接读） |
...
- **查 SDK 接口** → 直接 `read src/tl_driver/lib/include/tl_interface.h`（1420 行）或 `tl_types.h`（599 行），按区间读，不要在 `.so` 上搜。
